...
import static dev.wp.matter_manipulator.MMMod.LOGGER;
...
public class ShaderProgram implements AutoCloseable {
    ...
    public ShaderProgram(String domain, String vertShaderFilename, String fragShaderFilename) {
        ...
        try {
            ...
            LOGGER.error("Could not initialize shader program!", e);
            program = 0;
        }
    ...
    private static int loadAndCompileShader(int program, String domain, String filename, int shaderType) {
        ...
        final String code = loadFile(ResourceLocation.tryBuild(domain, filename));
        if (code == null) {
        ...
        if (GL20.glGetShaderi(shader, GL20.GL_COMPILE_STATUS) == GL11.GL_FALSE) {
            LOGGER.error("Could not compile shader {}: {}", filename, getShaderLogInfo(shader));
            GL20.glDeleteShader(shader);
            return 0;
        }
    ...
    private static int linkValidateShader(int program, int vertShader, int fragShader) {
        ...
        if (GL20.glGetProgrami(program, GL20.GL_LINK_STATUS) == GL11.GL_FALSE) {
            LOGGER.error("Could not link shader: {}", getProgramLogInfo(program));
        ...
        if (GL20.glGetProgrami(program, GL20.GL_VALIDATE_STATUS) == GL11.GL_FALSE) {
            LOGGER.error("Could not validate shader: {}", getProgramLogInfo(program));
            GL20.glDeleteProgram(program);
            return 0;
        }
    ...
    private static int loadAndCompileShader(int program, Path path, int shaderType) {
        ...
        if (GL20.glGetShaderi(shader, GL20.GL_COMPILE_STATUS) == GL11.GL_FALSE) {
            LOGGER.error("Could not compile shader {}: {}", path.toString(), getShaderLogInfo(shader));
            GL20.glDeleteShader(shader);
            return 0;
        }
    ...
    private static String loadFile(ResourceLocation resourceLocation) {
        try {
            ...
            final InputStream inputStream = Minecraft.getInstance().getResourceManager().getResource(resourceLocation)
                    .get().open();
            ...
            LOGGER.error("Could not load shader file!", e);
        }
    ...
    private static String loadFileFromPath(Path path) {
        try (BufferedReader reader = Files.newBufferedReader(path)) {
            ...
            LOGGER.error("Could not load shader file: {}", path, e);
        }
        return null;
    }
